...
export default class HomeScreen extends React.Component {
  ...
  render() {
		const { tickerAnalysisResults, tickerSymbols } = this.state;
    return (
        ...
        <ScrollView style={styles.container} contentContainerStyle={styles.contentContainer}>
          ...
          <SectionList
						...
						renderSectionHeader={({section: {title}}) => (
							...
							sections={
								Object.entries(tickerSymbols).map(
									([key, value]) => ({ title: key, data: [<NewsSection summary={value.map(x => x.headline).join(' ')} score={tickerAnalysisResults[key]} />] })
								)
							}
